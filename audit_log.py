...
DB_PATH = os.environ.get("AUDIT_LOG_DB", "audit_log.db")  # override to a persistent-disk path in prod
...
def _open_db() -> sqlite3.Connection:
    ...
    # WAL + busy_timeout let concurrent auditor writes queue instead of raising
    # "database is locked"; the BEGIN IMMEDIATE in append_event still serialises
    # writers so the hash chain stays intact.
...
def append_event(
    ...
    *,
    ...
    valid = {
        "flag_generated", "flag_viewed", "action_taken", "model_updated",
        # Authentication events (auth subsystem).
        "login_success", "login_failure", "logout",
        "account_locked", "mfa_challenge", "session_expired", "privilege_change",
    }
